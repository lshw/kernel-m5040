...
static void sm501_write_codec(struct sm501_audio *s, u8 addr, u16 data)
{
	...
	{	
		dev_dbg(s->dev, "%s ok: addr=%02x data=%04x\n",
		__func__, addr, data);
	}
}
...
#define AC97_DATA_FLOW_CONTROL 0x6a
	value = sm501_read_codec(audio_dev, AC97_DATA_FLOW_CONTROL);
	sm501_write_codec(audio_dev, AC97_DATA_FLOW_CONTROL, value | 0xc800);  //enable spdif in
...
static int snd_sm501_ac97_probe(struct platform_device *pdev)
{
	...
	/* Get irq number */
	audio_dev->irq = platform_get_irq(pdev, 0);
	if (!audio_dev->irq) {
...
struct sm502ac97_reg{
	unsigned char add;
	unsigned short data;
};


typedef union sm502ac97_type
{
	unsigned char val;
	struct sm502ac97_reg  reg;
}sm502ac97_t;


#define MAX_PINS 1
#define DEVNAME "sm502ac97"

static int sm502ac97_open(struct inode *inode, struct file *file)
{

	return 0;
}

static int sm502ac97_release(struct inode *inode, struct file *file)
{
	return 0;
}

ssize_t sm502ac97_write(struct file *file, const char __user *data,size_t len, loff_t *ppos)
{
	sm502ac97_t a;

	if (copy_from_user(&a, (sm502ac97_t __user *)data,sizeof(sm502ac97_t)))
	{
		return -EFAULT;
	}
//printk("-----a.reg.add:%02x a.reg.data:%04x-----\n",a.reg.add,a.reg.data);
	 sm501_write_codec(audio_dev, a.reg.add,a.reg.data);

	return 0;
}
ssize_t  sm502ac97_read(struct file *file, char __user * buf,size_t len, loff_t * ppos)
{
	sm502ac97_t a;
	unsigned int data;

	if (copy_from_user(&a, (sm502ac97_t __user *)buf,sizeof(sm502ac97_t)))
	{
		return -EFAULT;
	}
	data = sm501_read_codec(audio_dev, a.val);


	return data;
}

int sm502ac97_ioctl(struct inode * node,struct file *file,unsigned int cmd,unsigned long arg)
{
	return 0;
}

static const struct file_operations sm502ac97_fileops = {
	.owner   = THIS_MODULE,
	.write   = sm502ac97_write,
	.read    = sm502ac97_read,
	.open    = sm502ac97_open,
#ifdef CONFIG_64BIT
	.compat_ioctl   = sm502ac97_ioctl,
#else
	.ioctl   = sm502ac97_ioctl,
#endif
	.release = sm502ac97_release,
};

static struct cdev sm502ac97_cdev;  /* use 1 cdev for all pins */

static int major = 0;		/* default to dynamic major */
static int __init sm502ac97_init(void)
{
	int rc;
	dev_t devid;
	...
	int i=0;
		...
		printk(KERN_ERR " sm502ac97device_init.\n");
	if (major) {
		devid = MKDEV(major, 130);
		//rc = register_chrdev_region(devid, MAX_PINS, DEVNAME);
		rc = register_chrdev_region(devid,MAX_PINS, DEVNAME);
	} else {
		rc = alloc_chrdev_region(&devid, 0, MAX_PINS, DEVNAME);
		major = MAJOR(devid);
	}
	if(rc < 0){
		printk(KERN_ERR " sm502ac97_initalloc region filed.\n");
		return rc;
	}
	cdev_init(&sm502ac97_cdev, &sm502ac97_fileops);
	sm502ac97_cdev.owner = THIS_MODULE;


	cdev_add(&sm502ac97_cdev, devid, MAX_PINS);

		my_class = class_create(THIS_MODULE, "my_audio");
		if(IS_ERR(my_class))
		{
		...
		//ret=device_create( my_class, NULL, MKDEV(major, 0), "ch7028%d", i );
		ret=device_create(my_class, NULL, MKDEV(major, 0), "sm502ac97%d" ,"sm502ac97",i);
		printk(KERN_ERR " sm502ac97_device_major:%d,ret:%08x.\n",major,ret);


	return 0; /* succeed */
}

static void __exit sm502ac97_cleanup(void)
{
	cdev_del(&sm502ac97_cdev);

	unregister_chrdev_region(MKDEV(major, 0), MAX_PINS);
}

module_init(sm502ac97_init);
module_exit(sm502ac97_cleanup);
#endif
