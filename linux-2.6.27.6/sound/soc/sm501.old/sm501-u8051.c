...
int __devinit sm501_load_firmware(struct sm501_audio *s, const char* firmware)
{
	...
	/* Cold reset AC97 controller */
	value = SmRead32(AC97_CONTROL_STATUS);
	value = SET_FIELD(value, AC97_CONTROL_STATUS_COLD_RESET, RESET);
	SmWrite32(AC97_CONTROL_STATUS, value);

	msleep(1);

	value = SmRead32(AC97_CONTROL_STATUS);
	value = SET_FIELD(value, AC97_CONTROL_STATUS_COLD_RESET, NORMAL);
	...
	while (SmRead8( SRAM(init_count)) != 2) {
		if (i-- <= 0) {
			dev_err(s->dev, "%s: firmware timed out during reset init_count = 0x%x test_count = 0x%x \n", 
				__func__, SmRead8(SRAM(init_count)), SmRead8(SRAM(filler4)));
			return -ETIMEDOUT;
		}
		msleep(1);
	}
	dev_err(s->dev, "%s:  init_count = 0x%x test_count = 0x%x  \n", __func__, SmRead8(SRAM(init_count)),
				SmRead8(SmRead8(SRAM(filler4))));
