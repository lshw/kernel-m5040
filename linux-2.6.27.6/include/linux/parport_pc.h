...
static __inline__ unsigned char parport_pc_read_data(struct parport *p)
{
	...
	unsigned char val = readb (DATA (p));
#ifdef DEBUG_PARPORT
...
static inline void dump_parport_state (char *str, struct parport *p)
{
	...
	unsigned char ecr = readb (ECONTROL (p));
	//unsigned char dcr = inb (CONTROL (p));
	unsigned char dcr = readb (CONTROL (p));
	//unsigned char dsr = inb (STATUS (p));
	unsigned char dsr = readb (STATUS (p));
	...
	for (i=0; i<2; i++) {
		...
		dcr = i ? priv->ctr : writeb (CONTROL (p));
