...
static unsigned char __iomem *sm50x_base_reg = NULL;
static unsigned char __iomem *sm50x_base_mem = NULL;
...
const unsigned char __iomem * SmIoAddress()
{
	return (sm50x_base_reg);
}
...
void SmMemset(unsigned long nOffset, unsigned char val, int count)
{
...
EXPORT_SYMBOL_GPL(sm501_date_gpio);
...
EXPORT_SYMBOL_GPL(sm501_dir_gpio);
...
static resource_size_t sm501_get_memory_size()
{
			      ...
			      unsigned long req_freq)
{
	...
	dev_info(sm->dev, "clock %08lx, mode %08lx\n",
		  clock, mode);
...
static int sm501_fill_usbdev(struct device *dev, struct platform_device *pdev)
{
...
static int sm501_register_display(struct sm501_devdata *sm)
{
	...
	sm501_create_mem(sm, &pdev->resource[2], 0, sm50x_mem_size-USB_DMA_BUFFER_SIZE);
	sm501_create_irq(sm, &pdev->resource[3]);

	return sm501_register_device(sm, pdev);
}

static int sm501_register_i2c(struct sm501_devdata *sm)
{
	struct platform_device *pdev;
	pdev = sm501_create_subdev(sm, "sm501-i2c", 1);
	if (!pdev)
		return -ENOMEM;
	sm501_create_irq(sm, &pdev->resource[0]);
...
static int sm501_register_uart(struct sm501_devdata *sm)
{
	...
	pdev = sm501_create_subdev(sm, "sm501-uart", 1);
	if (!pdev)
		return -ENOMEM;
...
static int sm501_register_audio(struct sm501_devdata *sm)
{
	...
	pdev = sm501_create_subdev(sm, "sm501-audio", 1);
	if (!pdev)
		return -ENOMEM;
	sm501_create_irq(sm, &pdev->resource[0]);
			    ...
			    struct sm501_initdata *init)
{
	...
	RegValue = FIELD_SET(RegValue, DRAM_CONTROL,LOCAL,RESET);
#endif
	SmWrite32(DRAM_CONTROL, RegValue);
#ifdef CONFIG_SYS_HAS_CPU_LOONGSON2
	RegValue = FIELD_SET(RegValue, DRAM_CONTROL,LOCAL,NORMAL);
	SmWrite32(DRAM_CONTROL, RegValue);	
#endif
...
//			FIELD_SET(RegValue, SYSTEM_CTRL, PCI_MASTER, START)|
	...
	RegValue = SmRead32(GPIO_CONTROL_HIGH);
...
static int sm501_init_dev(struct sm501_devdata *sm, struct device *dev)
{
	...
	dev_info(sm->dev, "SM502/SM107 At %p: Version %08x, %ld Mb, IRQ %d\n",
		 sm->regs, SmRead32( DEVICE_ID),(unsigned long)sm50x_mem_size >> 20, sm->irq);
	...
	if (sm->platdata) {
		...
		if (pdata->init) {
			...
			printk(KERN_INFO"devices is 0x%x\n", pdata->init->devices);
...
#if 0
			if (pdata->init->devices & SM501_USE_I2C) {
				printk("Using SM501 I2C module\n");
				sm501_register_i2c(sm);
			}
#endif			
		}
	}
			   ...
			   const struct pci_device_id *id)
{
	...
	/* check register */
	sm->regs_claim = request_mem_region(sm->io_res->start,0x100, "sm501");
	...
	/* mapping register */
	sm->regs = sm50x_base_reg = ioremap(pci_resource_start(dev, 1), pci_resource_len(dev, 1));
//	printk("--2-------sm50x_base_reg:%08x \n",sm50x_base_reg);
...
static int sm501_chr_ioctl(struct inode *inode, struct file *flip, u32 cmd, unsigned long arg)
{
	switch (cmd)
	{
...
MODULE_DESCRIPTION("SM50x/SM107 Core Driver");
