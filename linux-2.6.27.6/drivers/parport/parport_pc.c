...
static int clear_epp_timeout(struct parport *pb)
{
	...
	writeb (r | 0x01, STATUS (pb)); /* Some reset by writing 1 */
	//outb (r & 0xfe, STATUS (pb)); /* Others by writing 0 */
	writeb (r & 0xfe, STATUS (pb)); /* Others by writing 0 */
	r = parport_pc_read_status(pb);
					       ...
					       const void *buf, size_t length)
{
	...
	while (left) {
		...
		unsigned char ecrval = readb (ECONTROL (port));
		int i = 0;
...
static int parport_ECR_present(struct parport *pb)
{
	...
	if ((inb (ECONTROL (pb)) & 0x3) == (r & 0x3)) {
		...
		writeb (r ^ 0x2, CONTROL (pb)); /* Toggle bit 1 */
...
//printk("---p->base:%lx---",p->base);
	...
	base_res = request_mem_region(base, 3, p->name);
	if (!base_res)
	//	goto out4;
#endif 
//printk("--mj:%s--out4-\n",__func__);
...
{
	int count = 0;
//printk("--mj:%s--partport_pc_probe55-\n",__func__);
#if 0
	...
	if (parport_pc_probe_port(0x278, 0x678, autoirq, autodma, NULL))
		count++;
#endif
	if (parport_pc_probe_port(0xffffffffbe0000e0, 0x0, autoirq, autodma, NULL))
		count++;
...
#if 0 //mj add 
/* S3C2410 ¿ª·¢°åÀ©Õ¹²¢¿Ú16C552µÄlow-levelÇý¶¯
 *
 * Author: Allen He<wingwind@sina.com>
 *
 *
 *
 * The extended parallel port provides one port at a fixed address
 * with 8 output data lines (D0 - D7), 1 output control line (STROBE)
 * and 1 input status line (BUSY) able to cause an interrupt.
 */

//#define MODULE  //HYL Add 


//#include <asm/irq.h>
//#include <asm/hardware.h>
//#include <asm/arch/cpu_s3c2410.h>
#include <linux/parport_2410_16c552.h>
#define INT_EINT8_23 34
static struct parport *this_port = NULL;

 unsigned char
parport_16c552_read_data(struct parport *p)
{
	unsigned long flags;
	unsigned char data;
	...
	//cli();
	data=*((unsigned char *)(p->base+0x0)) ;
	//local_restore_flags(flags);
printk("-----------------hah--mj:%s-data:%08x--\n",__func__,data);
	return data;
}

static void
parport_16c552_write_data(struct parport *p, unsigned char data)
{
	unsigned long flags;
parport_16c552_read_data(p);
printk("-----------------hah--mj:%s---%08x---%02x\n",__func__,(p->base),data);
	//save_flags(flags);
	//cli();
	*((unsigned char *)(p->base+0x0))=data;
	//restore_flags(flags);
	return;
}
...
unsigned char parport_16c552_read_control(struct parport *p)
{
	unsigned long flags;
	...
	//save_flags(flags);
	//cli();
	control=*((unsigned char *)(p->base+0x2));
	//restore_flags(flags);
	return control;

}

void parport_16c552_write_control(struct parport *p, unsigned char d)
{
	...
	//save_flags(flags);
	//cli();
	*((unsigned char *)(p->base+0x2))=d;
	//restore_flags(flags);
	return;
}

static unsigned char
parport_16c552_frob_control(struct parport *p, unsigned char mask,
			   unsigned char val)
{
	unsigned char old = parport_16c552_read_control(p);
	parport_16c552_write_control(p, (old & ~mask) ^ val);
	return old;
}

static unsigned char
parport_16c552_read_status(struct parport *p)
{
printk("-----------------hah--mj:%s-data:%08x--\n",__func__,*((unsigned char *)(p->base+0x1)));
	return (*((unsigned char *)(p->base+0x1)));
}

static void
parport_16c552_init_state(struct pardevice *d, struct parport_state *s)
{
}

static void
parport_16c552_save_state(struct parport *p, struct parport_state *s)
{
}

static void
parport_16c552_restore_state(struct parport *p, struct parport_state *s)
{
}

static void
//parport_16c552_interrupt(int irq, void *dev_id, struct pt_regs *regs)
parport_16c552_interrupt(void *dev_id)
{
//printk("-----------------hah--mj:%s---\n",__func__);
...
static void
parport_16c552_enable_irq(struct parport *p)
{
	enable_irq(INT_EINT8_23);
}

static void
parport_16c552_disable_irq(struct parport *p)
{
	disable_irq(INT_EINT8_23);
}

static void
parport_16c552_data_forward(struct parport *p)
{
	unsigned long flags;
	////save_flags(flags);
	//cli();
	parport_16c552_frob_control(p,0x20,0x0);
	//restore_flags(flags);
}

static void
parport_16c552_data_reverse(struct parport *p)
{
	unsigned long flags;
	//save_flags(flags);
	//cli();
	parport_16c552_frob_control(p,0x20,0x20);
	//restore_flags(flags);
}

static void
parport_16c552_inc_use_count(void)
{
//	MOD_INC_USE_COUNT;
}

static void
parport_16c552_dec_use_count(void)
{
//	MOD_DEC_USE_COUNT;
}

static struct parport_operations parport_16c552_ops = {
	parport_16c552_write_data,
	parport_16c552_read_data,

	parport_16c552_write_control,
	parport_16c552_read_control,
	parport_16c552_frob_control,

	parport_16c552_read_status,

	parport_16c552_enable_irq,
	parport_16c552_disable_irq,

	parport_16c552_data_forward,
	parport_16c552_data_reverse,

	parport_16c552_init_state,
	parport_16c552_save_state,
	parport_16c552_restore_state,

	parport_16c552_inc_use_count,
	parport_16c552_dec_use_count,
...
//printk("--1---------------hah--mj:%s---\n",__func__);
		/*·ÖÅäÄÚ´æÇøÓò*/
		if(check_mem_region(brd_16c552_parportbase,0x3))//0x800 is a number defined as will.
		{
			printk("16c552 driver:memory already in use\n");
			return -EBUSY;
		}
//printk("--2---------------hah--mj:%s---\n",__func__);
		request_mem_region(brd_16c552_parportbase,0x3,"parport_16c552");		
		
///printk("--3---------------hah--mj:%s---\n",__func__);
		p = parport_register_port(brd_16c552_parportbase,INT_EINT8_23,
					  PARPORT_DMA_NONE,
					  &parport_16c552_ops);
//printk("--4---------------hah--mj:%s--,%s-,%08x\n",__func__,p->name,p);
		if (!p)
			return 0;
		//if (request_irq(INT_EINT8_23, parport_16c552_interrupt,SA_SHIRQ, p->name, p)) {
		if (request_irq(INT_EINT8_23, parport_16c552_interrupt,0, p->name, p)) {
		//	parport_unregister_port (p);
			return 0;
		}
	
/*		// initialize the parport as a input device with disabling irq. 
		save_flags(flags);
		cli();
		parport_16c552_frob_control(p,0x3f,0x24);		
		restore_flags(flags);
*/


		this_port = p;
		printk(KERN_INFO "%s: s3c2410 brd extended with 16c552, port using irq\n", p->name);
...
MODULE_AUTHOR("Wingwind");
MODULE_DESCRIPTION("Parport Driver for 16c552 parellel Port");
MODULE_SUPPORTED_DEVICE("S3c2410 extending 16c552 Parallel Port");
MODULE_LICENSE("GPL");

int
init_module(void)
{
	return parport_16c552_init() ? 0 : -ENODEV;
}
...
cleanup_module(void)
{
	...
	release_mem_region(brd_16c552_parportbase,0x3,);	
}
#endif
...
MODULE_LICENSE("GPL");
//module_init(parport_16c552_init)
