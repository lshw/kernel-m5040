...
#if defined(CONFIG_LEMOTE_FULONG2F) /* FIXUP_CS5536_RTC */
...
int rtc_mytest()
{
	unsigned int year1, mon, day, hour, min, sec;
	unsigned long flags;

	spin_lock_irqsave(&rtc_lock, flags);

	printk("sec %d, min %d,hour %d,day %d,mon %d, year1 %d\n",RTC_SECONDS,RTC_MINUTES,RTC_HOURS,RTC_DAY_OF_MONTH,RTC_MONTH,RTC_YEAR);
	do {
