...
unsigned int uca =1;
unsigned long fb_start = 0xffffffffUL, fb_end = 0xffffffffUL;
/*
...
static inline int uncached_access(struct file *file, unsigned long addr)
{
	...
	if (file->f_flags & O_SYNC)
		return 1;
#if defined(CONFIG_CPU_LOONGSON2) && defined(CONFIG_64BIT)
...
static int __init uca_setup(char *str)
{
       char tmp[100], *end;

       uca = 1;
       strncpy(tmp, str, 100);
       fb_start = simple_strtoul(tmp, &end, 16);
       if (end && *end) {
                       end ++;
                       fb_end = fb_start + simple_strtoul(end, NULL, 16) ;
       }

       return 0;
}

__setup("uca=", uca_setup);
